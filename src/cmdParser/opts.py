...
definitions = [
    ...
    # training
    ('epochs',           (int,   120,     "Number of epochs to train.")),
    ...
    ('optimizer',        (str,   'adam', "Optimizer: sgd, adam")),
    ('learning-rate',    (float, None,   "Optimizer learning rate.")),
    ('momentum',         (float, None,   "Momentum for SGD optimizer.")),
    ('decay',            (float, None,   "Learning rate decay, sgd")),
    ('beta_1',           (float, None,   "Adam beta-1")),
    ('beta_2',           (float, None,   "Adam beta-2")),
    ('metrics_F1',       {'default': False, 'action': 'store_true',
                            'help': "F1-score metric monitoring"}),
    ('metrics_precision',   {'default': False, 'action': 'store_true',
                            'help': "precision metric monitoring"}),
    ('metrics_recall',      {'default': False, 'action': 'store_true',
                            'help': "recall metric monitoring"}),
    ('metrics_jaccard',     {'default': False, 'action': 'store_true',
                            'help': "jaccard metric monitoring"}),


    # files
    ('load_weights',     (str, '', "Load model weights from specified file to initialize training.")),
    ('data_dir',         (str,   '.',    "Directory containing patientXX/ directories.")),
    ('output_dir',       (str,   '.',    "Directory to write output data.")),
    ('weight_file',      (str,   'weights-final.hdf5', "File to write final model weights.")),
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Train U-Net to segment left ventricles from cardiac Ultrasound images.")
